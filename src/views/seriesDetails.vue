  ...
  <div>
    ...
    <body>
      <div class="tituloquiz">
        <h1 v-if="choosenSerie">
          {{ choosenSerie.title }}
        </h1>
        <p class="subtitulo" v-if="choosenSerie">
          {{ choosenSerie.release }}
        </p>
      </div>
      <br />
      <b-container>
        <b-row>
          <b-col>
            ...
            <div style="padding-bottom: 20px;">
              <br />
              <b-img v-if="choosenSerie" :src="choosenSerie.image" style="width:100%;height: auto;padding-bottom: 20px;"></b-img>
              <b-form-checkbox
            ...
            <h1>Sinopse</h1>
            <p v-if="choosenSerie">
              {{ choosenSerie.description }}
            </p>
            <br />
            <h1>Diretor</h1>
            <p v-if="choosenSerie">{{ choosenSerie.realizador }}</p>
            <br />
            <h1>Escritor</h1>
            <p v-if="choosenSerie">{{ choosenSerie.writers }}</p>
            <br />
            <h1>Atores</h1>
            <p v-if="choosenSerie">{{ choosenSerie.actors }}</p>
          </b-col>
        </b-row> </b-container
      ><br />
      <!-- commentarios -->
...
export default {
  ...
  mounted(){
    this.getSerieInfo(this.getChoosenSerie);
  },
  computed: {
    ...mapGetters(["getChoosenSerie"])
  },
  methods: {
    ...mapActions(["serieInfo"]),

    async getSerieInfo(title){
      const response = await this.serieInfo(title);

      if(response.data.success == true){
        this.choosenSerie = response.data.serie
      }
    }
  },
};
